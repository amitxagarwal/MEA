    ...
    public class MeaClient : IMeaClient
    {
        ...
        public async Task<ResultOrHttpError<string, Error>> PostAsync(Uri uri, StringContent stringContent)
        {
            ...
            if (!response.IsSuccessStatusCode)
            {
                ...
                {
                    var errorFromResponse = await response.Content.ReadAsStringAsync().ConfigureAwait(false);
                    var error = new Error(_correlationId, new string[] { "An error occured while creating journal note in Momentum Core System" }, "MEA");
                    Log.ForContext("CorrelationId", _correlationId).Error($"Error Occured while creating journal note in Momentum Core System : {errorFromResponse}");
                ...
                Log.ForContext("CorrelationId", _correlationId).Error($"Error Occured while creating journal note in Momentum Core System {errorResponse}");
