...
namespace Kmd.Momentum.Mea.Tests.Caseworker
{
    public class CaseworkerTests
    {
        ...
        public async Task GetAllCaseworkersSuccess()
        {
            ...
            mockResponseData.Add(JsonConvert.SerializeObject(new ClaseworkerData("testId1", "TestDisplay1", "givenname", "middlename", "initials", "test@email.com", "1234567891", "", "description", true, true)));
            mockResponseData.Add(JsonConvert.SerializeObject(new ClaseworkerData("testId2", "TestDisplay2", "givenname", "middlename", "initials", "test@email.com", "1234567891", "", "description", true, true)));
            ...
            helperHttpClientMoq.Setup(x => x.GetAllCaseworkerDataFromMomentumCoreAsync(new Uri($"{_configuration.Object["KMD_MOMENTUM_MEA_McaApiUri"]}/punits/0d1345f4-51e0-407e-9dc0-15a9d08326d7/caseworkers")))
                .Returns(Task.FromResult((IReadOnlyList<ClaseworkerData>)mockResponseData));


            var caseworkerService = new CaseworkerService(helperHttpClientMoq.Object, _configuration.Object);
            var responseData = mockResponseData.Select(x => JsonConvert.DeserializeObject<ClaseworkerData>(x)).ToList();
