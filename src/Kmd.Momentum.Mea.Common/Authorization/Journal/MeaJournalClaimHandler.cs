...
namespace Kmd.Momentum.Mea.Common.Authorization.Journal
{
    public class MeaJournalClaimHandler : AuthorizationHandler<MeaJournalClaimRequirement>
    {
        ...
        public MeaJournalClaimHandler(IConfiguration configuration)
        {
        ...
        protected override Task HandleRequirementAsync(AuthorizationHandlerContext context, MeaJournalClaimRequirement requirement)
        {
            ...
            if (!context.User.HasClaim(c => c.Type == requirement.JournalAudience))
                return Task.CompletedTask;

            // If user does not have the tenant claim, get out of here
            if (!context.User.HasClaim(c => c.Type == requirement.JournalTenant))
                return Task.CompletedTask;

            // If user does not have the scope claim, get out of here
            if (!context.User.HasClaim(c => c.Type == requirement.JournalScope))
                return Task.CompletedTask;

            // Split the audience, tenants, scope string into an array
            var audience = context.User.FindFirst(c => c.Type == MeaJournalClaimAttributes.JournalAudienceClaimTypeName).Value.Split(' ');
            var tenant = context.User.FindFirst(c => c.Type == MeaJournalClaimAttributes.JournalTenantClaimTypeName).Value.Split(' ');
