...
try {
    ...
    foreach ($testFolder in Get-ChildItem "./test/*.Tests") {
        ...
        try {
            ...
            ($env:ASPNETCORE_ENVIRONMENT=$env:ENVIRONMENT) | Write-Host "build: Testing project in '$testFolder'" & dotnet test -c Release --logger trx --verbosity="$BuildVerbosity" --no-build --no-restore
            if($LASTEXITCODE -ne 0) { exit 3 }
        }
        finally {
            Pop-Location
